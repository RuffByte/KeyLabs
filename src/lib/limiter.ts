...
export async function rateLimitByIp({
  ...
  await rateLimitByKey({ // Call the generic rate limiting function
...
export async function rateLimitByKey({
  ...
  if (tracker.expiresAt < Date.now()) { // Check if the tracker has expired
  ...
  if (tracker.count > limit) { // Check if the limit has been exceeded
